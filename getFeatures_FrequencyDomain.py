...
input_N_start = 1
input_N_end = 51
df_output_url = 'Data/Features/220912_FrequencyDomain.xlsx'
...
for n in range(input_N_start, input_N_end+1):
    ...
    ecg_url = 'Data/ClipSituation_CSVfile/N{}/'.format(n)
    ...
    # for stress in range(1): # Run for four situation
        ...
        # for i in range(1): # one stress situation have 10 data
            ...
            if len(rpeakindex)<=2: #若只抓到小於等於2點的Rpeak，會無法算HRV參數，因此將參數設為0
                ...
                # plt.xlim(0,150)
                # plt.xlabel('Time (s)')
                # plt.title(columns[stress]+' N'+str(n))
                ...
                # plt.subplot(plt_len,1,2)
                # plt.plot(rrinterval_resample, 'black',marker='o')
                # # plt.xlim(0,150)
                # plt.ylabel('RRI\n(ms)')
                # plt.xlabel('Time (s)')
                # plt.ylim(200,1000)
                ...
                # plt.plot(rrinterval_true, 'black', marker='o')
                # # plt.xlim(0,150)
                # plt.ylabel('Re RRI\n(ms)')
                # plt.xlabel('Time (s)')
                # plt.ylim(200,1000)
